import ActorSheetPF2e from "../../systems/pf2e/module/actor/sheet/npc.js";
...
class LootSheetNPC extends ActorSheetPF2e {
    ...
    static SOCKET = "module.lootsheetnpcpf2e";
    ...
    get template() {
        ...
        if (!game.user.isGM && this.actor.limited) return path + "npc-sheet.html";
        return "modules/lootsheetnpcpf2e/template/npc-sheet.html";
    }
    ...
    static get defaultOptions() {
        ...
        mergeObject(options, {
            classes: ["sheet actor npc npc-sheet loot-sheet-npc pf2e updatedNPCSheet"],
            width: 850,
            height: 750
        });
        return options;
    ...
    async getData() {
        ...
        let lootsheettype = await this.actor.getFlag("lootsheetnpcpf2e", "lootsheettype");
        if (!lootsheettype) await this.actor.setFlag("lootsheetnpcpf2e", "lootsheettype", "Loot");
        lootsheettype = await this.actor.getFlag("lootsheetnpcpf2e", "lootsheettype");

        
        let priceModifier = 1.0;
        if (lootsheettype === "Merchant") {
            priceModifier = await this.actor.getFlag("lootsheetnpcpf2e", "priceModifier");
            if (!priceModifier) await this.actor.setFlag("lootsheetnpcpf2e", "priceModifier", 1.0);
            priceModifier = await this.actor.getFlag("lootsheetnpcpf2e", "priceModifier");
        }
    ...
    async _merchantInventoryUpdate(event, html) {
        ...
        let clearInventory = game.settings.get("lootsheetnpcpf2e", "clearInventory");
        ...
        for (let i = 0; i < shopQtyRoll.result; i++) {
            ...
            if (!newItem || newItem === null) {
                ...
                newItem = await compendiumitems.getEntity(rollResult.results[0].resultId);
                if (!newItem || newItem === null) {
    ...
    _createRollTable() {
        ...
        const pack = game.packs.find(p => p.collection === "pf2e.items");
    ...
    _buyItem(event) {
        ...
        if (game.user.actorId) {
            ...
            let d = new QuantityDialog((quantity) => {
                    ...
                    console.log("LootSheetNPC", "Sending buy request to " + targetGm.name, packet);
                    game.socket.emit(LootSheetNPC.SOCKET, packet);
                },
                {
    ...
    _lootItem(event) {
        ...
        if (game.user.actorId) {
            ...
            let d = new QuantityDialog((quantity) => {
                    ...
                    console.log("LootSheetNPC", "Sending loot request to " + targetGm.name, packet);
                    game.socket.emit(LootSheetNPC.SOCKET, packet);
                },
                {
                    acceptLabel: "Loot"
    ...
    async _priceModifier(event) {
        ...
        let priceModifier = await this.actor.getFlag("lootsheetnpcpf2e", "priceModifier");
        ...
        let d = new Dialog({
            ...
            buttons: {
             one: {
              icon: '<i class="fas fa-check"></i>',
              label: "Update",
              callback: () => this.actor.setFlag("lootsheetnpcpf2e", "priceModifier", document.getElementById("price-modifier-percent").value / 100)
             },
             two: {
              icon: '<i class="fas fa-times"></i>',
              label: "Cancel",
    ...
    _prepareItems(actorData) {
        ...
        for (let i of actorData.items) {
            ...
            else if (i.type === "treasure") features.loot.items.push(i);
            else features.loot.items.push(i);
        }

        // Assign and return
        //actorData.features = features;
        actorData.items = features;
        //console.log(this.actor);
    }
    ...
    _prepareGMSettings(actorData) {
        ...
        for (let c in currencySplit) {
            if (owners.length)
                currencySplit[c].value = Math.floor(actorData.data.currency[c].value / owners.length);
            else
                currencySplit[c] = 0
        }
...
/**
 * Register a hook to convert any spell created on an actor with the LootSheetNPC sheet to a consumable scroll.
 */
Hooks.on('preCreateOwnedItem', (actor, item, data) => {
    ...
    // If the actor is using the LootSheetNPC then check in the item is a spell and if so update the name.
    if ((actor.data.flags.core || {}).sheetClass === "pf2e.LootSheetNPC") {
        if (item.type === "spell") {
...
Hooks.once("init", () => {
    ...
    game.settings.register("lootsheetnpcpf2e", "clearInventory", {
    ...
    function chatMessage (speaker, owner, message, item) {
        if (game.settings.get("lootsheetnpcpf2e", "buyChat")) {
            message =   `
    ...
    function errorMessageToActor(target, message) {
        game.socket.emit(LootSheetNPC.SOCKET, {
            type: "error",
            targetId: target.id,
            message: message
        });
    ...
    function moveItem(source, destination, itemId, quantity) {
        ...
        if (item.data.quantity.value < quantity) {
            quantity = item.data.quantity.value;
        }

        let newItem = duplicate(item);
        const update = {_id: itemId, "data.quantity.value": item.data.quantity.value - quantity};

        if (update["data.quantity.value"] === 0) {
        ...
        newItem.data.quantity.value = quantity;
    ...
    function transaction(seller, buyer, itemId, quantity) {
        ...
        let sellerModifier = seller.getFlag("lootsheetnpcpf2e", "priceModifier");
    ...
    game.socket.on(LootSheetNPC.SOCKET, data => {
